...
# p <- ggplot(accu_hand_north, aes(x = light_dir, y = ))
```

```{r}
# Calculating accuracy per gender and respond
accu_gender_m <- df_long %>% 
  filter(accuracy == "yes",
         gender == "m") %>% 
  group_by(landform, land_dir, light_dir) %>% 
  summarise(accu_count = n()) %>% 
...
accu_exp_some_north <- accu_exp %>%
  group_by(light_dir) %>%
  summarise(mean_some = mean(some_pct))

accu_exp_no_north <- accu_exp %>%
  group_by(light_dir) %>%
  summarise(mean_no = mean(no_pct))
